...
class VitisAcceleratorWriter(VitisWriter):
    ...
    def write_datamover(self, model):
        ...
        f = open(os.path.join(filedir, '../templates/vitis_accelerator/alveo/axi_mm2s/mm2s.h'))
        os.makedirs(os.path.join(model.config.get_output_dir(), 'firmware', 'axi_mm2s'), exist_ok=True)
        fout = open(f'{model.config.get_output_dir()}/firmware/axi_mm2s/mm2s.h', 'w')

        for line in f.readlines():
            if 'hls-fpga-machine-learning insert bus_width' in line:
                newline = f'#define BUS_WIDTH {inp_axi_t.width * inp.size()}\n'
            else:
                newline = line
            fout.write(newline)
        f.close()
        ...
        f = open(os.path.join(filedir, '../templates/vitis_accelerator/alveo/axi_mm2s/mm2s.cpp'))
        fout = open(f'{model.config.get_output_dir()}/firmware/axi_mm2s/mm2s.cpp', 'w')
        for line in f.readlines():
            newline = line
            fout.write(newline)
        f.close()
        fout.close()
        copyfile(os.path.join(filedir, '../templates/vitis_accelerator/alveo/axi_mm2s/script.tcl'), 
                    os.path.join(model.config.get_output_dir(), 'firmware', 'axi_mm2s', 'script.tcl'))

        #######################
        # datamover_s2mm
        #######################
        filedir = os.path.dirname(os.path.abspath(__file__))
        f = open(os.path.join(filedir, '../templates/vitis_accelerator/alveo/axi_s2mm/s2mm.h'))
        os.makedirs(os.path.join(model.config.get_output_dir(), 'firmware', 'axi_s2mm'), exist_ok=True)
        fout = open(f'{model.config.get_output_dir()}/firmware/axi_s2mm/s2mm.h', 'w')

        for line in f.readlines():
            if 'hls-fpga-machine-learning insert bus_width' in line:
                newline = f'#define BUS_WIDTH {out_axi_t.width * out.size()}\n'
            else:
                newline = line
            fout.write(newline)
        f.close()
        ...
        f = open(os.path.join(filedir, '../templates/vitis_accelerator/alveo/axi_s2mm/s2mm.cpp'))
        fout = open(f'{model.config.get_output_dir()}/firmware/axi_s2mm/s2mm.cpp', 'w')
        for line in f.readlines():
            newline = line
            fout.write(newline)
        f.close()
        fout.close()
        copyfile(os.path.join(filedir, '../templates/vitis_accelerator/alveo/axi_s2mm/script.tcl'), 
                    os.path.join(model.config.get_output_dir(), 'firmware', 'axi_s2mm', 'script.tcl'))
    ...
    def write_axi_wrapper(self, model):
        ...
        '''
        # inp_axi_t and out_axi_t are instance of FixedPrecisionType
        inp_axi_t, out_axi_t, inp, out = self.vitis_accelerator_config.get_corrected_types()
        indent = '    '
        ...
        for line in f.readlines():
            if 'MYPROJECT' in line:
                ...
                if self.vitis_accelerator_config.get_interface() == 'axi_stream':
                    newline += f'typedef {self._ap_fixed_to_string(inp_axi_t)} T_in;\n'
                    newline += f'typedef {self._ap_fixed_to_string(out_axi_t)} T_out;\n'
                    newline += (
                        ...
                        + indent
                        + '{ return stream << "{ data: " << in.data.to_string() << ", last: " << in.last.to_string() << " }" << std::endl; }\n'
                        + indent
                    ...
                    newline += (
                        ...
                        + indent
                        + '{ return stream << "{ data: " << out.data.to_string() << ", last: " << out.last.to_string() << " }" << std::endl; }\n'
                        + indent
                        + 'operator float() const {return this->data;}\n'
                        + indent
                        + 'operator double() const {return this->data;}\n'
                    ...
                    newline += f'typedef {self._ap_fixed_to_string(inp_axi_t)} input_axi_t;\n'
                    newline += f'typedef {self._ap_fixed_to_string(out_axi_t)} output_axi_t;\n'
            else:
                newline = line
            fout.write(newline)
        f.close()
    ...
    def _ap_fixed_to_string(self, ap_fixed_type):
        args = [ap_fixed_type.width, ap_fixed_type.integer, 
                f'AP_{ap_fixed_type.rounding_mode}', 
                f'AP_{ap_fixed_type.saturation_mode}', ap_fixed_type.saturation_bits]
        args = ','.join([str(arg) for arg in args])
        typestring = 'ap_{signed}fixed<{args}>'.format(signed='u' if not ap_fixed_type.signed else '', args=args)
        return typestring
    
    def write_axi_stream_wrapper(self, model):
        # inp_axi_t and out_axi_t are instance of FixedPrecisionType
        inp_axi_t, out_axi_t, inp, out = self.vitis_accelerator_config.get_corrected_types()
        indent = '    '
        ...
        filedir = os.path.dirname(os.path.abspath(__file__))
        f = open(os.path.join(filedir, '../templates/vitis_accelerator/alveo/axi_stream/myproject_axi_stream.h'))
        os.makedirs(os.path.join(model.config.get_output_dir(), 'firmware', 'axi_stream'), exist_ok=True)
        fout = open(f'{model.config.get_output_dir()}/firmware/axi_stream/{model.config.get_project_name()}_axi_stream.h', 'w')
        ...
        # myproject_axi.cpp
        #######################
        f = open(os.path.join(filedir, '../templates/vitis_accelerator/alveo/axi_stream/myproject_axi_stream.cpp'))
        fout = open(f'{model.config.get_output_dir()}/firmware/axi_stream/{model.config.get_project_name()}_axi_stream.cpp', 'w')
        ...
        f.close()
        fout.close()
        copyfile(os.path.join(filedir, '../templates/vitis_accelerator/alveo/axi_stream/script.tcl'), 
                    os.path.join(model.config.get_output_dir(), 'firmware', 'axi_stream', 'script.tcl'))

    def prepare_build_script(self, model):
        filedir = os.path.dirname(os.path.abspath(__file__))
        copyfile(os.path.join(filedir, '../templates/vitis_accelerator/alveo/build_xo.sh'), 
                    os.path.join(model.config.get_output_dir(), 'build_xo.sh'))
        copyfile(os.path.join(filedir, '../templates/vitis_accelerator/alveo/build_hw.sh'), 
                    os.path.join(model.config.get_output_dir(), 'build_hw.sh'))
        copyfile(os.path.join(filedir, '../templates/vitis_accelerator/alveo/config.cfg'), 
                    os.path.join(model.config.get_output_dir(), 'config.cfg'))
        copyfile(os.path.join(filedir, '../templates/vitis_accelerator/alveo/xrt.ini'), 
                    os.path.join(model.config.get_output_dir(), 'xrt.ini'))
    ...
    def modify_build_script(self, model):
        ...
        newfile = f'{model.config.get_output_dir()}/build_prj_axi_stream.tcl'
        f = open(oldfile)
        fout = open(newfile, 'w')
    ...
    def write_hls(self, model):
        ...
        self.write_axi_stream_wrapper(model)
        self.write_datamover(model)
        self.modify_build_script(model)
        self.prepare_build_script(model)
