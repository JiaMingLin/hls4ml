...
class NeuralNetworkOverlay(Overlay):
    ...
    def allocate_mem(self, X_shape, y_shape, X_dtype=np.float32, y_dtype=np.float32, trg_in=None, trg_out=None):
    ...
    def predict(self, X, y_shape, X_dtype=np.uint16, y_dtype=np.uint32, debug=False, profile=False, encode=None, decode=None):
        ...
        """
        self.allocate_mem(X_shape=X.shape, y_shape=y_shape, X_dtype=X_dtype, y_dtype=y_dtype )
        if profile:
            timea = datetime.now()
        ...
        self.mm2s.start(self.input_buffer, in_length)
        self.s2mm.call(self.output_buffer)
        if debug:
